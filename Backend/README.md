...
graph TD
    ...
    GEMINI[Gemini AI / FastAPI System]:::external
    ...
    IAM -- "Session tokens / OTPs" --> REDIS

    USER -- "Profiles/Reports" --> DB
    USER -- "Data Caching" --> REDIS
    USER -- "Avatars/Media Files" --> MINIO
    USER -- "AI Analysis/Inference" --> GEMINI
```

### Execution Flow Details

1. **Client Request**: A client (mobile app or web frontend) makes a standard HTTP request to the API Gateway.
2. **Gateway Routing**: The `api-gateway` validates the request, verifies JWT tokens via auth middleware, and marshals the JSON payload into Protocol Buffers.
3. **gRPC Invocation**: The Gateway acts as a gRPC client, calling the appropriate internal microservice (`iam-service` for auth/identity, `user-service` for profile/content/reports).
4. **Service Execution**:
   - The microservice processes the business logic.
   - For fast, ephemeral data (like sessions and OTPs), it reads/writes to `Redis`.
   - For persistent state (users, reports), it interacts with `PostgreSQL`.
   - For file assets (e.g., profile pictures, audio, video), the `user-service` manages uploads via `MinIO`.
   - AI evaluations are fired off to the Python FastAPI system or Gemini via the User Service.
...
- **High-Performance Storage Layers**:
  - **PostgreSQL**: Relational database capabilities for ACID compliant data.
  - **Redis**: In-memory data store for lightning-fast caching and OTP/Session management.
  - **MinIO**: S3-compatible Object Storage for handling user media uploads (video, audio, images) securely.
- **AI Integration**: Deep integration with the E-VEDA Python inference API and Gemini for intelligent user data processing and educational insights.
...
The `api-gateway` exposes the following main REST endpoints. All user and report endpoints require a valid JWT Bearer token.
...
- `POST /api/verify-otp` - Validate email OTP.
- `POST /api/reset-password` - Set new password.

### User Management (`user-service`)
- `GET /api/user` - Fetch full user profile.
- `PUT /api/user` - Update user profile information.
- `GET /api/user/completion` - Get profile completion percentage.
- `GET /api/user/picture-name` - Get basic avatar and name.
- `POST /api/user/profile-picture` - Upload a new profile picture.
- `POST /api/user/update-password` - Change account password.

### Reports & AI (`user-service`)
- `POST /api/report/upload/audio` - Upload audio file to MinIO for analysis.
- `POST /api/report/upload/video` - Upload video file to MinIO for analysis.
- `POST /api/report/create` - Trigger the AI analysis pipeline and generate a report.
- `GET /api/reports` - Fetch a paginated list of historical reports.
- `GET /api/report` - Fetch a specific report's full details (using `?id=`).
- `GET /api/ai/global-trends` - Retrieve aggregated emotional trends.
- `GET /api/ai/insight` - Retrieve a daily AI-generated insight.
...
1. **Protocol Buffers First**: Any new feature requiring cross-service communication starts in the `proto/` directory. Define your service and messages, then run `make proto` to generate the Go stubs.
2. **Implement Handlers**: Write the business logic in the respective microservice (`iam-service` or `user-service`).
3. **Expose in Gateway**: Add the REST route mapping in `api-gateway/internal/gateway/router.go` and implement the handler.
4. **Code Style**: Ensure you write standard, idiomatic Go. Use `gofmt` and `make tidy` before committing.
